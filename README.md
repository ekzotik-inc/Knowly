...
Каждый пользователь получает персонального stylized companion-персонажа. Character Engine хранит в профиле характер и параметры внешности, а Mini App использует персонажа на Home, при создании теста, во время прохождения и на экране результата.

Поддерживаются характеры `Cute`, `Funny`, `Romantic`, `Confident`, `Mysterious` и `Chaotic`. Визуальные параметры включают форму головы, оттенок кожи, причёску, цвет волос, глаза, брови, одежду, аксессуар, очки, головной убор и палитру. Контекстные состояния включают `default`, `playful`, `happy`, `sad`, `proud`, `thinking` и `excited`; на результатах и игровых событиях меняются также поза и реплика companion-персонажа.

MVP использует composable SVG-рендерер с градиентами и анимационными CSS-классами. Это даёт единый узнаваемый стиль без хранения тяжёлых персональных изображений и оставляет возможность позже заменить renderer на полноценные 3D-модели, не меняя API конфигурации персонажа.
...
Сейчас реализованы Telegram auth exchange, каталог продуктов, создание invoice link в XTR, pre-checkout validation, successful payment processing, entitlements, рабочий игровой MVP и Personalized Character System: создание теста, публикация deep link, прохождение по вопросам, серверный подсчёт результата, premium-разбор и персонализируемый companion с контекстными эмоциями.
