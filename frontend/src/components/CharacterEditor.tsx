...
const choices: Choice[] = [
  { key: "gender", label: "Пол и персонаж", items: [["feminine", "Она"], ["masculine", "Он"]] },
...
export function CharacterEditor({ config, onChange, onSave, saving = false, onboarding = false, saveLabel }: CharacterEditorProps) {
  ...
  return <section className={`character-editor ${onboarding ? "onboarding-editor" : ""}`}>
    <div className="editor-heading"><div><strong>{onboarding ? "Создай своего персонажа" : "Измени своего персонажа"}</strong><small>{onboarding ? "Сначала имя и пол — затем Knowly сам подберёт основу персонажа" : "Ты можешь изменить его в любой момент"}</small></div><span>{onboarding ? "первый шаг" : "персонализация"}</span></div>
    ...
    <button type="button" className="secondary wide" disabled={saving} onClick={onSave}>{saving ? "Сохраняем…" : saveLabel ?? (onboarding ? "Создать моего персонажа" : "Сохранить персонажа")}</button>
  </section>;
}
