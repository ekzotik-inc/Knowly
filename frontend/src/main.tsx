...
import { Character, defaultCharacter, masculineCharacter } from "./components/Character";
import { CharacterEditor } from "./components/CharacterEditor";
import type { CharacterConfig, CharacterPair } from "./components/character-types";
import "./styles.css";
...
type Profile = { id: string; display_name: string; avatar_url: string | null; character: CharacterConfig; characters?: CharacterPair; locale: "ru" | "en" | "uz"; result_visibility: string; sound_enabled: boolean; haptic_enabled: boolean };
...
function App() {
  ...
  const [characters, setCharacters] = useState<CharacterPair>({ feminine: defaultCharacter, masculine: masculineCharacter });
  const [activeCompanion, setActiveCompanion] = useState<"feminine" | "masculine">("feminine");
  const [switchBurst, setSwitchBurst] = useState(0);
  const character = characters[activeCompanion];
  ...
  const companionTitle = activeCompanion === "masculine" ? "Твой Knowly-бро" : "Твоя Knowly-подруга";
  const companionPronoun = activeCompanion === "masculine" ? "Он" : "Она";
  const companionSelected = activeCompanion === "masculine" ? "выбран" : "выбрана";

  function selectCompanion(next: "feminine" | "masculine") {
    if (next === activeCompanion) return;
    setActiveCompanion(next);
    setSwitchBurst((value) => value + 1);
    haptic("light");
  }
  ...
  async function saveCharacter() {
    ...
    try {       const profile = await api<Profile>("/api/v1/profile", { method: "PUT", body: JSON.stringify({ display_name: firstName, avatar_url: null, character, characters }) }); setCharacters(profile.characters ?? characters); setCharacterOpen(false); notify("Оба персонажа сохранены"); haptic("light");
 }
  ...
  return <main className="app-shell home-screen"><header className="topbar home-topbar"><div className="brand-lockup"><span className="brand-mark">✦</span><div><strong>knowly</strong><small>social quiz game</small></div></div><div className="profile-orb">{firstName.slice(0, 1).toUpperCase()}</div></header><section className="home-hero"><div className="hero-copy"><span className="eyebrow">ПРИВЕТ, {firstName.toUpperCase()}</span><h1>Кто знает тебя<br /><em>по-настоящему?</em></h1><p>Создай игру о себе. Отправь друзьям. Смотри, кто угадал.</p><div className="hero-actions"><button className="primary-button" onClick={() => { setScreen("create"); haptic("light"); }}>Создать тест <span>→</span></button><button className="round-button" onClick={openBot} aria-label="Открыть бота">↗</button></div></div><div className="hero-character pair-scene"><div className="hero-bubble">ваша пара<br /><b>♡</b></div><div className={`pair-character pair-feminine ${activeCompanion === "feminine" ? "is-selected" : ""}`}><Character config={characters.feminine} emotion="happy" pose="bounce" /></div><div className={`pair-character pair-masculine ${activeCompanion === "masculine" ? "is-selected" : ""}`}><Character config={characters.masculine} emotion="happy" pose="bounce" /></div></div><div className="hero-glow glow-one" /><div className="hero-glow glow-two" /></section><section className="stats-row"><div><span>Уровень</span><strong>{progress?.level ?? 1}</strong></div><div><span>XP собрано</span><strong>{progress?.xp ?? 0}</strong></div><div><span>Тестов создано</span><strong>{progress?.tests_created ?? tests.length}</strong></div></section><section className="section-block character-section"><div key={switchBurst} className={`switch-burst ${switchBurst ? "is-visible" : ""}`} aria-hidden="true"><span className="burst-heart heart-a">♥</span><span className="burst-heart heart-b">♥</span><span className="burst-sparkle sparkle-a">✦</span><span className="burst-sparkle sparkle-b">✧</span></div><div className="section-title"><div><span className="eyebrow">ТВОЙ COMPANION</span><h2>{companionTitle}</h2></div><button className="text-button" onClick={() => setCharacterOpen((value) => !value)}>{characterOpen ? "Свернуть" : "Изменить"} <span>→</span></button></div>{characterOpen ? <CharacterEditor config={character} activeCompanion={activeCompanion} onCompanionChange={selectCompanion} onChange={(next) => setCharacters((pair) => ({ ...pair, [activeCompanion]: next }))} onSave={() => void saveCharacter()} saving={characterSaving} /> : <div className="companion-preview pair-preview"><div className="pair-thumbnails"><button className={activeCompanion === "feminine" ? "pair-thumb active" : "pair-thumb"} onClick={() => { selectCompanion("feminine"); setCharacterOpen(true); }} aria-label="Настроить её"><Character config={characters.feminine} emotion="happy" compact /></button><button className={activeCompanion === "masculine" ? "pair-thumb active" : "pair-thumb"} onClick={() => { selectCompanion("masculine"); setCharacterOpen(true); }} aria-label="Настроить его"><Character config={characters.masculine} emotion="happy" compact /></button></div><div><strong>Два companion — одна история</strong><p>{companionPronoun} сейчас {companionSelected} для настройки. Оба реагируют на игру и живут в одном стиле.</p></div><button className="mini-arrow" onClick={() => setCharacterOpen(true)}>→</button></div>}</section><section className="section-block"><div className="section-title"><div><span className="eyebrow">ТВОИ ИГРЫ</span><h2>Тесты, которыми делятся</h2></div><span className="section-counter">{tests.length}</span></div>{tests.length === 0 ? <div className="empty-state"><div className="empty-icon">✦</div><strong>Здесь появится твоя первая игра</strong><p>Начни с трёх вопросов — это займёт пару минут.</p><button className="text-button" onClick={() => setScreen("create")}>Начать создание <span>→</span></button></div> : <div className="test-list">{tests.map((test, index) => <article className="test-card-new" key={test.id}><div className="test-card-index">{String(index + 1).padStart(2, "0")}</div><div className="test-card-copy"><strong>{test.title}</strong><span>{test.question_count} вопросов <i>·</i> опубликован</span></div><div className="test-card-actions"><button onClick={() => shareTest(test)} aria-label="Поделиться тестом">↗</button><button onClick={() => copyTest(test)} aria-label="Скопировать ссылку">⧉</button></div></article>)}</div>}</section><section className="bot-card"><div className="bot-card-symbol">✦</div><div><span className="eyebrow">НЕ ТЕРЯЙСЯ В ЧАТАХ</span><strong>Knowly живёт и в боте</strong><p>Запускай игры из Telegram и возвращайся к результатам в один тап.</p></div><button className="mini-arrow" onClick={openBot}>↗</button></section>{products.length > 0 ? <section className="section-block"><div className="section-title"><div><span className="eyebrow">ДОПОЛНЕНИЯ</span><h2>Открыть premium</h2></div><span className="section-counter">⭐</span></div>{products.map((product) => <article className="product-card-new" key={product.code}><div className="product-icon">✦</div><div><strong>{product.title}</strong><p>{product.description}</p></div>{entitlements.some((item) => item.entitlement_key === product.entitlement_key && item.active) ? <span className="owned">Доступно</span> : <button onClick={() => void buy(product)} disabled={busy}>{product.stars} ⭐</button>}</article>)}</section> : <div className="free-mode-note"><span>♡</span><div><strong>Сейчас всё бесплатно</strong><p>Мы тестируем Knowly. Premium появится позже — игровой процесс уже доступен.</p></div></div>}{error && <div className="notice error">{error}</div>}{busy && <div className="loading-line"><i /> Загружаем твой Knowly…</div>}{toast && <div className="toast">{toast}</div>}</main>;
}
