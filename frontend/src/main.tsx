...
function App() {
  ...
  const companionTitle = character.gender === "masculine" ? "Твой Knowly-бро" : character.gender === "neutral" ? "Твой Knowly companion" : "Твоя Knowly-подруга";
  const companionPronoun = character.gender === "masculine" ? "Он" : character.gender === "neutral" ? "Они" : "Она";
  ...
  async function saveCharacter() {
    ...
    try { const profile = await api<Profile>("/api/v1/profile", { method: "PUT", body: JSON.stringify({ display_name: firstName, avatar_url: null, character }) }); setCharacter(profile.character); setCharacterOpen(false); notify("Персонаж сохранён"); haptic("light"); }
