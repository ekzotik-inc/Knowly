...
function App() {
  ...
  const validDrafts = drafts.filter((draft) => {
    ...
    return draft.text.trim() && options.length >= 2 && options.length <= 4 && new Set(options).size === options.length && draft.correct_option.trim() && options.includes(draft.correct_option.trim());
  });
  ...
  async function answer(option: string) {
    ...
    setSelectedOption(option); setBusy(true); setError(""); haptic("light");
    try {
  ...
  function addOption(index: number) { setDrafts((items) => items.map((item, itemIndex) => itemIndex === index && item.options.length < 4 ? { ...item, options: [...item.options, ""] } : item)); }
  function removeOption(index: number, optionIndex: number) { setDrafts((items) => items.map((item, itemIndex) => {
    ...
    return { ...item, options, correct_option: item.correct_option === removed ? "" : item.correct_option };
  })); }
  ...
  function goHome() { setScreen("home"); setPublicTest(null); setResult(null); setResultDetail(null); setResultReveal(false); setSessionId(""); setSelectedOption(null); }
